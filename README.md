...
>
> **`python3` / `ffmpeg` / `git` / `curl` 这些系统依赖如果没装，`install.sh` 会询问并自动调用包管理器装好**（macOS 用 Homebrew，缺 brew 时连 brew 一起装；Linux 自动识别 apt / dnf / yum / pacman / zypper）。你不需要先手工准备它们。
...
IndexTTS2 首次加载模型（尤其 CPU 模式）需要 2–5 分钟，属正常现象。`bash start.sh logs tts` 查看加载进度。
