...
1. **本地 TTS 必须用户手动启动**：`main.py` 不再尝试自动启动 9800；用户需在 indextts env 中跑 `python tts_service/server.py`。
2. **api/domain/clients 三层不能跨越**：api 调 domain，domain 调 clients；不允许 api 直接 import httpx 或 api 直接读写文件。
...
4. **情绪叠加 0.6 折算**：`domain/matcher.py` 中当 LLM 诊断主情绪与参考音一致时把 emo_alpha 乘 0.6 防爆音——不要在 api 层重复实现。
5. **`emo_vector` 偷渡协议**：`clients/tts.py` 把 `voice` 字段写成 `[EMO:[v0..v7]|alpha]base64:...`；`tts_service/server.py` 入口处用正则剥离。两端必须同步。
6. **`webapp/api/_progress.py`**：跨 router 共享的后台任务进度字典，进程重启会丢失。
7. **API 白名单优先**：智能匹配候选池由 `domain/matcher._select_candidate_pool` 决定——只要存在 `is_api_safe=true` 的素材，就只用这批；否则用全集。
8. **前端禁缓存靠时间戳**：`webapp/app.py` 把所有 `.js` / `.css` 引用替换为 `?t={timestamp}`。
9. **`outputs/` 永远不会自动清理**：合成产物会无限累积。
10. **ffmpeg 信任系统 PATH**：不再绑内置 ffmpeg；用户需自行 `brew install ffmpeg`。

## 配置文件约定

`webapp/config/config.json`（首次启动时自动创建在 `webapp/` 同级的 `config/` 下）结构：
```jsonc
{
  "llm": {
    "active_type": "ollama",
