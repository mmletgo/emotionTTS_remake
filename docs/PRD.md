...
> - 旧版本配置自动迁移到 quick 模式
> - Windows 一键启动批处理（一键启动.bat / 启动TTS服务.bat）+ core/pyenv 嵌入式 Python
> - lite.html / lite_server.py（精简版独立部署）
> - indexTTS_Server 中完整 IndexTTS2 模型源代码（改为依赖用户 indextts env，仅保留 `indexTTS_Server/tts_server.py` 轻量 wrapper）
...
### 0.4 范围声明
本 PRD 覆盖：Web UI 主版本（index.html）、Web 中枢（FastAPI on 9880）、本地 IndexTTS2 服务（FastAPI on 9800，轻量 wrapper `indexTTS_Server/tts_server.py`，依赖用户的 indextts env）、启动器（`python main.py`）。
...
┌──────────────────────────────────────────────────────┐
│ main.py (启动器，无端口)                              │
│   └─ subprocess: app.py → 监听 9880 (Web 中枢)       │
└──────────────────────────────────────────────────────┘
                       │
        ┌──────────────┴──────────────────┐
        │ Web 中枢 9880 (app.py)          │
...
| `core/config/config.json` | 全局配置（LLM / TTS） |
...
- 如需本地 TTS 模式，用户需另在自己的 indextts env 中运行 `python indexTTS_Server/tts_server.py`
...
And 同时提示用户如需本地 TTS 模式请在 indextts env 中启动 tts_server.py
```
...
And 调 MultiTTSProvider.analyze_emotion，使用 EMOTION_ANALYSIS_PROMPT
...
**Background**
- 用户已在自己的 indextts env 中运行 `python indexTTS_Server/tts_server.py`
...
### 4.4 9800 本地 TTS 服务（indexTTS_Server/tts_server.py）
| Method | Path | 用途 |
| --- | --- | --- |
| GET | `/health` | 健康检查 |
...
| 平台 | 启动方式 |
| --- | --- |
| macOS / Linux / Windows | `python main.py` 启动 Web 中枢；本地 TTS 需在自己的 indextts env 里另开终端 `python indexTTS_Server/tts_server.py` |
...
6. **本地模式必须先起 9800**：当 `tts.type=local` 时，前端探活和合成都依赖 9800 健康；否则 verify_active 返回 tts_status=error。本仓库不会自动拉起该服务，用户需在 indextts env 中手动启动 `tts_server.py`。
