...
| **任务进度（task_progress）** | 服务端进程内变量，键为 `char_id`、`{char_id}_append` 或 `{char_id}_relabel`；每个值含 progress / msg / status / stage 字段 |
...
└──────────────────────────────────────────────────────┘
        ...
        │   │    └─ library_builder → emotion_tagger  │
        │   ├─ library_editor / matcher                │
        │   │    └─ library_editor.relabel_emotions_logic │
        │   ├─ emotion_tagger / synthesizer            │
        │   ├─ text_splitter                           │
        │                                              │
...
```gherkin
Given 用户在独立全屏 BuildCharacterView 中填写：
  - char_name = "芙宁娜"
  - avatar = furina.png（可选，拖拽或点选）
  - files = [voice_clip_01.wav, voice_clip_02.mp3]（拖拽区上传）
  - min_silence_len = 0.8 秒
  - enable_llm_tagging = true（AI 情绪打标开关，默认开）
When 用户点击"开始构建"，前端发起 POST /api/characters (multipart/form-data)
Then 后端：
  1. 生成 char_id 形如 char_<8 位十六进制>
  2. 创建 characters/{char_id}/，保存 avatar 与原始上传文件
  3. 在 BackgroundTasks 中提交 build_character_dataset(char_id, enable_llm_tagging)，立即同步返回 {status: "success", char_id}
And 前端在 BuildCharacterView 的四阶段进度卡片中每 ~2 秒轮询 GET /api/progress/{char_id}
And 进度响应含 stage 字段（'slicing' | 'asr' | 'tagging' | 'writing' | null），前端据此高亮当前阶段卡片
```

#### Scenario 2.5.2 自动切片流水线（含 LLM 情绪打标）
```gherkin
Given webapp.domain.library_builder.build_character_dataset 后台任务已启动
When 后台任务开始执行
Then 系统按顺序执行四个阶段：

  【阶段 1：切片 slicing，进度 5–15%】
  ...
  【阶段 3：LLM 情绪打标 tagging，进度 50–90%】（enable_llm_tagging=true 时执行，否则跳过）
  7. 调用 domain.emotion_tagger.tag_items_sync(items, llm_cfg, batch_size=15)
     · 每批 15 段拼成 JSON 数组发给 LLM，使用 BATCH_EMOTION_TAGGING_PROMPT
     · LLM 返回每段的 {primary, complex, intensity}，写回对应 item.emotion
     · 单批 LLM 失败不影响其他批（该批 emotion 保持默认"平"）
     · LLM 完全不可用时抛 EmotionTaggerError，整个任务标记 error

  【阶段 4：写入 writing，进度 90–100%】
  8. 每条有效片段生成 item：id 连续递增、filename = "voice_lib/{char_id}_NNNN.wav"
     · enable_llm_tagging=true 时 emotion 为 LLM 打标结果
     · enable_llm_tagging=false 时 emotion 默认 {primary:"平", complex:"", intensity:"Medium"}
     · duration 精确到 0.01s
  9. 写入 library.json
  10. 删除所有原始上传文件（temp_*）

And task_progress[char_id] 进度 JSON 含 stage 字段，取值对应当前阶段：
  · 'slicing'（5-15%）→ 'asr'（15-50%）→ 'tagging'（50-90%）→ 'writing'（90-100%）
And 完成后 stage = null，status = "success"
And 失败时 task_progress[char_id].status = "error"，msg 包含异常信息
```

#### Scenario 2.5.4a 新建完成总览
```gherkin
Given task_progress[char_id].status 变为 "success"
When 前端轮询拿到 success
Then BuildCharacterView 切换到"完成总览"视图，显示：
  · 成功入库的片段总数
  · 情绪分布（各 primary 标签的段数）
And 用户可选择"进入详情"（跳转到该角色的素材库视图）或"继续新建"（重置 BuildCharacterView）
And 前端同时触发角色列表刷新（GET /api/characters），新角色卡片出现在角色网格
```
...
Then 后端返回 {progress: 0-100, msg: "<消息>", status: "running"|"success"|"error", stage: 'slicing'|'asr'|'tagging'|'writing'|null}
And 若 task_id 未注册，返回 {progress: 0, msg: "等待中...", status: "running", stage: null} (兜底)
```
...
```gherkin
Given 用户已选中某个角色，在素材库详情页的右侧抽屉（CharacterFormSheet）上传新音频
When 前端 POST /api/characters/{char_id}/append (multipart, files=[...], min_silence_len=0.8, enable_llm_tagging=true)
...
And 后台任务执行与 build_character_dataset 相同的四阶段流水线（切片 → ASR → LLM 打标 → 写入），但：
  · 读取现有 library.json，新 item.id 从 max(已有 id) + 1 起递增
  · 新文件命名规则：voice_lib/{char_id}_append_NNNN_<4 位 uuid>.wav
  · 新条目追加（而非覆盖）到 items 数组
  · enable_llm_tagging=false 时跳过打标阶段，emotion 保持默认"平"
And 前端轮询 /api/progress/{char_id}_append，返回含 stage 字段的进度 JSON（同 Scenario 2.5.3）
```
...
#### Scenario 2.7.6 批量 AI 情感分析（一键打标，前端逐条）
```gherkin
...
#### Scenario 2.7.8 后端批量 AI 重标情绪（relabel）
```gherkin
Given 用户在素材库详情页点击"AI 重标情绪"按钮
When 前端 POST /api/characters/{char_id}/relabel
  Body: {"item_ids": [1, 2, 3]}  // 指定范围；null 表示对全部 items 重标
Then 后端在 BackgroundTasks 中启动后台重标任务，task_id = "{char_id}_relabel"
And 立即返回 {status: "success", task_id: "{char_id}_relabel"}
And 后台调用 domain.library_editor.relabel_emotions_logic(char_id, item_ids, llm_cfg)：
  · 加载 library.json，按 item_ids 过滤（null 则取全量）
  · 调用 domain.emotion_tagger.tag_items_sync(items, llm_cfg, batch_size=15)，每批 15 段发给 LLM
  · 写回 library.json（仅更新 emotion 字段）
And 前端轮询 GET /api/progress/{char_id}_relabel，stage 固定为 'tagging'
And 完成后 status = "success"，前端刷新素材库详情

Note：
  · "AI 重标情绪"（后端批量，relabel）与"一键 AI 情绪分析"（前端逐条，Scenario 2.7.6）是两个独立能力
  · relabel 直接落盘，不需要用户二次"保存更改"
  · item_ids=null 时等价于对当前角色全量素材重标
```
...
| POST | `/api/characters` | 创建角色（切片 → ASR → LLM 打标 → 写入）；multipart 含 `enable_llm_tagging: bool = Form(True)` | `domain.characters.stage_uploads_for_create` + 后台 `library_builder.build_character_dataset` → `emotion_tagger.tag_items_sync` |
...
| POST | `/api/characters/{char_id}/append` | 追加素材（四阶段后台任务）；multipart 含 `enable_llm_tagging: bool = Form(True)` | `domain.characters.stage_uploads_for_append` + 后台 `library_builder.append_character_dataset` → `emotion_tagger.tag_items_sync` |
...
| POST | `/api/characters/{char_id}/relabel` | 后端批量 LLM 重标情绪；body `{"item_ids": int[] \| null}`（null = 全量）；返回 `{status, task_id}` | 后台 `domain.library_editor.relabel_emotions_logic` → `emotion_tagger.tag_items_sync` |
...
| GET | `/api/progress/{task_id}` | 轮询后台任务进度；返回 `{progress, msg, status, stage}`，stage 取值 `'slicing'\|'asr'\|'tagging'\|'writing'\|null` | `api._progress.get` |
