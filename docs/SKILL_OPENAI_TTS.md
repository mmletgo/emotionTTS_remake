...
> ⚠️ **调用模式硬性约定：必须串行，一句一请求**
> 服务端 TTS 是单卡单进程串行推理。合成多句台词时**必须**等上一句的 HTTP 响应返回（拿到 WAV 字节）后，再发下一句的请求；不允许任何形式的并发（`asyncio.gather` / `Promise.all` / 线程池 / 多协程齐发 / 多 curl 后台 &）。需要长段落时，先按标点切分成单句，顺序合成后在客户端本地拼接（ffmpeg / pydub），不要试图让服务端并行。违反这条只会更慢，并增加超时与显存爆裂风险。详见下方"设计与使用约定"第 8 条。
...
8. **严格串行，禁止并发**（**强制要求**）：服务端 TTS 推理是单卡串行，并发请求只会在队列里阻塞，不会更快，反而会拉长每一句的等待时间、放大超时与显存压力的概率。**长文本必须按句切分，一句合成完、拿到 WAV 后，再发下一个 `/v1/audio/speech` 请求**。绝不要用 `asyncio.gather` / `Promise.all` / 线程池 / 批量发请求等方式同时投递多句。需要拼成一段长音频时，按顺序合成完所有片段，再在客户端用 ffmpeg / pydub 等本地工具拼接，不要把"加速"寄希望于服务端并发。
